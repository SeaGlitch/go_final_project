...
func addTaskH(w http.ResponseWriter, r *http.Request, db *sql.DB) {
	...
	if err := json.NewDecoder(r.Body).Decode(&task); err != nil {
		...
		//		response := map[string]string{"error": "Couldn't unJSON"}
	...
	default:
		match, _ := regexp.MatchString(`^d \d+$`, task.Repeat) // Проверка на соответствие регулярному выражению
		if !match {
...
func putTaskH(w http.ResponseWriter, r *http.Request, db *sql.DB) {
	...
	//Памятка: Нужно вынести в отдельную функцию
	now := time.Now()
	nowSt := time.Now().Format("20060102")
	nowTD, err := time.Parse("20060102", nowSt)
	if err != nil {
		http.Error(w, `{"error": "ошибка}`, http.StatusBadRequest)
		return
	}
	...
	default:
		...
		match, _ := regexp.MatchString(`^d \d+$`, task.Repeat) // Проверка на соответствие
		if !match {
			response := map[string]string{"error": "правило повторения указано в неправильном формате"}
			json.NewEncoder(w).Encode(response)
			return
...
func getTaskH(w http.ResponseWriter, r *http.Request, db *sql.DB) {
	...
	if err == sql.ErrNoRows {
		http.Error(w, `{"error": "ошибка}`, http.StatusNotFound)
		return
	} else if err != nil {
		http.Error(w, `{"error": "ошибка}`, http.StatusInternalServerError)
		return
